	...
	for {
		...
		case "deposit":
			...
			if index == 0 {
				...
				fmt.Printf("抵押 %f ETH, %f Lava \n", value, depositB)
				fmt.Printf("获取share %f , %f  \n", MintA, MintB)
		...
		case "withdraw":
			...
			fmt.Printf("赎回 %f ETH, %f Lava \n", removedA, removedB)

		}
		fmt.Printf("eth/lava交易对常量为 %f \n", core.LavaEthPair.Invariant)
		fmt.Printf("eth的最新储量为 %f, lava的最新储量为 %f \n", core.LavaEthPair.APool, core.LavaEthPair.BPool)
		fmt.Printf("eth的最新share为 %f, lava的最新share为 %f \n", core.LavaEthPair.AShare, core.LavaEthPair.BShare)
